...
def angulos_2D_zy2_I(base,punto):
    orientacion=punto-base
    if orientacion[0]<0:
        angulo_x = np.arctan(orientacion[1] / orientacion[0])-np.pi
        if orientacion[1]<0:
...
def main(robotIP, PORT):
    ...
    tts.say("hello I am NAO, I am ready to imitate you")
    bol=True
    bol_I=True
    bandera=False
    bandera_I=False
    ...
    while True:
        ...
        if skeleton.frame.visible:
            ...
            sombra_zI = Vector_Codo_I - Vector_Hombro_IF
            sombra_zI[2] = 0
            ...
            if Modo_Gestos:
                ...
                Giro_codo = Gestos(Vector_Hombro_DF, Vector_Codo_D, Vec_U_X)

                complemento = angulo_lateral2(np.array(Vector_Hombro_DF), np.array(Vector_Codo_D), np.array(sombra_z))
                if complemento < 20:
                    Rota_codo_D = rotacion(np.array(Vector_Codo_D), np.array(Vector_Muneca_D)) * (np.pi / 180) + HombroFrente_D
                ...
                Rota_codo_I = angulos_2D_zy2_I(np.array(Vector_Codo_I), np.array(Vector_Muneca_I)) * (np.pi / 180)
                Giro_codo_I = Gestos(Vector_Hombro_IF, Vector_Codo_I, Vec_U_NegX)

                complemento_I = angulo_lateral2(np.array(Vector_Hombro_IF), np.array(Vector_Codo_I), np.array(sombra_zI))
                if complemento_I < 20:
                    Rota_codo_I = -rotacion(np.array(Vector_Codo_I), np.array(Vector_Muneca_I)) * (np.pi / 180) - HombroFrente_I

                if Giro_codo_I < 20:
                    HombroFrente_I = 0
                    Rota_codo_I = -rotacion(np.array(Vector_Codo_D), np.array(Vector_Muneca_D)) * (np.pi / 180)
                ...
                if altura_D < altura_I - .06:
                    ...
                    # Mapeo de los angulos optenidos del esqueleto a las articulaciones de NAO
                    angles = [HombroLateral_D, HombroFrente_D, Rota_codo_D, AnguloCodo_D,
                              HombroLateral_I, HombroFrente_I, Rota_codo_I, AnguloCodo_I,
